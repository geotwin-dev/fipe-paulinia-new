...
const MapFramework = {
    ...
    finalizarDesenho: function (opts = { descartarTemporario: false }) {
        ...
        // Oculta o botão de finalizar desenho e mostra os botões de quadra/lote
    ...
    iniciarDesenhoQuadra: function () {
        ...
        // Mostra o botão de finalizar desenho e esconde os botões de quadra/lote
        $('#btnFinalizarDesenho').removeClass('d-none');
        $('#btnIncluirPoligono').addClass('d-none');
        $('#btnIncluirLinha').addClass('d-none');
        ...
        this.desenho.listenerRightClick = this.map.addListener('rightclick', (e) => {
            ...
            // Marcar como salvando para evitar duplo clique
            this.desenho.salvando = true;
            
            // Desabilitar temporariamente o listener
            if (this.desenho.listenerRightClick) {
                this.desenho.listenerRightClick.remove();
            }
    ...
    salvarDesenho: function (camada, identificador = '') {
        ...
        if (this.desenho.salvando && this.desenho.temporario === null) {
            console.log('⚠️ Tentativa de salvar desenho já processado, ignorando');
            return;
        }
            ...
            success: (response) => {
                ...
                try {
                    ...
                    if (resultado.status === 'sucesso') {
                        ...
                        // Para lotes e quadras, mantém o modo de desenho ativo para permitir desenhar mais
                        if (this.desenho.tipoAtual === 'polilinha' || this.desenho.tipoAtual === 'poligono') {
                            // Limpa o temporário mas mantém o modo de desenho
                            this.desenho.temporario = null;
                            this.desenho.pontos = [];
                            // Recria os listeners para permitir continuar desenhando
                            this.recriarListenersDesenho();
                        }
                    } else {
    ...
    recriarListenersDesenho: function () {
        // Remove listeners antigos se existirem
        if (this.desenho.listenerClick) this.desenho.listenerClick.remove();
        if (this.desenho.listenerRightClick) this.desenho.listenerRightClick.remove();

        // Recria os listeners baseado no tipo de desenho atual
        if (this.desenho.tipoAtual === 'poligono') {
            this.recriarListenersPoligono();
        } else if (this.desenho.tipoAtual === 'polilinha') {
            this.recriarListenersPolilinha();
        }
    },

    recriarListenersPoligono: function () {
        // Clique esquerdo no mapa → adiciona ponto
        this.desenho.listenerClick = this.map.addListener('click', (e) => {
            const latLng = e.latLng;
            this.desenho.pontos.push(latLng);
                ...
                google.maps.event.addListener(this.desenho.temporario, 'rightclick', (e) => {
                    ...
                    if (typeof e.vertex === 'number') {
                        if (path.getLength() <= 3) {
                            ...
                            path.removeAt(e.vertex);
                            this.desenho.pontos.splice(e.vertex, 1);
                        }
                    }
                });
            } else {
    ...
    recriarListenersPolilinha: function () {
        // Clique esquerdo no mapa → adiciona ponto
        this.desenho.listenerClick = this.map.addListener('click', (e) => {
            const ponto = e.latLng;
        ...
        this.desenho.listenerRightClick = this.map.addListener('rightclick', (e) => {
            ...
            const pathLength = this.desenho.temporario.getPath().getLength();
            if (pathLength < 2) {
                alert("Você precisa de pelo menos 2 pontos.");
                return;
            }
    ...
    controlarOpacidade: function (value) {
        // Controla opacidade das quadras
        ...
        // Controla opacidade dos loteamentos
    ...
    inserirMarcadorPersonalizado: function (latLng) {
        ...
        // Pega a quadra do input
        let quadraDoInput = $('#inputQuadraAtual').val().trim();
        if (!quadraDoInput) {
            alert("Por favor, informe a quadra no campo correspondente.");
            return;
        }
        ...
        // Inicialmente marca como loading (laranja)
        const corInicial = '#FF8C00'; // Laranja para loading

        // Cria HTML do marcador
        let el = document.createElement('div');
        el.style.height = '32px';
        el.style.padding = '0 10px';
        el.style.background = corInicial;
        ...
        el.innerHTML = numeroLote + ' <i class="fas fa-spinner fa-spin"></i>';
        ...
        let marker = new google.maps.marker.AdvancedMarkerElement({
            ...
            content: el,
            gmpClickable: true,
            zIndex: MapFramework.obterZIndexMarcador(corInicial)
        });
        marker.setMap(this.map);
        ...
        // Usa a quadra do input (já validada acima)
        loteElementoSelecionado = $('.opcao-lote.selected');
        marker.quadra = quadraDoInput;
        ...
        this.salvarMarcadorNoBanco(latLng, idQuadra, numeroLote, marker, correspondeAoLoteSelecionado);
    },

    salvarMarcadorNoBanco: function(latLng, idQuadra, numeroMarcador, marcadorElement, correspondeAoLoteSelecionado) {
        // Usa a quadra do input (já validada na função anterior)
        const quadraDoInput = $('#inputQuadraAtual').val().trim();
        loteElementoSelecionado = $('.opcao-lote.selected');
        const quadraSelecionada = quadraDoInput;
        
        // Calcula a cor final baseada se corresponde ao lote selecionado
        ...
        console.log('- Corresponde ao lote selecionado:', correspondeAoLoteSelecionado);
        console.log('- Cor final enviada para o banco:', corFinal);
        */
        ...
        $.ajax({
            ...
            data: {
                ...
                cor: corFinal // Cor final (verde ou vermelho)
            },
            success: function(response) {
                try {
                    ...
                    if (resultado.status === 'sucesso' && resultado.id) {
                        ...
                        // Atualiza a cor do marcador baseada no resultado
                        MapFramework.atualizarCorMarcador(marcadorElement, correspondeAoLoteSelecionado);
                        
                        // Auto-incremento do input se for numérico
                        MapFramework.autoIncrementarInputLote(numeroMarcador);
                        ...
                        // Erro ao salvar - marca como cinza
                        MapFramework.atualizarCorMarcador(marcadorElement, false, 'erro');
            ...
            error: function() {
                // Erro de conexão - marca como cinza
                MapFramework.atualizarCorMarcador(marcadorElement, false, 'erro');
    ...
    // Função para atualizar a cor do marcador baseada no status
    atualizarCorMarcador: function(marcadorElement, correspondeAoLoteSelecionado, status = 'sucesso') {
        const el = marcadorElement.content;
        if (!el) return;
        
        let cor, icone, texto;
        
        if (status === 'erro') {
            // Erro ao salvar - cinza
            cor = '#808080';
            icone = '';
            texto = marcadorElement.numeroMarcador;
        } else if (correspondeAoLoteSelecionado) {
            // Sucesso e lote existe na lista - verde
            cor = '#32CD32';
            icone = '';
            texto = marcadorElement.numeroMarcador;
        } else {
            // Sucesso mas lote não existe na lista - vermelho
            cor = '#FF0000';
            icone = '';
            texto = marcadorElement.numeroMarcador;
        }
        
        // Atualiza o estilo e conteúdo
        el.style.background = cor;
        el.innerHTML = texto + ' ' + icone;
        
        // Atualiza o z-index baseado na nova cor
        const novoZIndex = MapFramework.obterZIndexMarcador(cor);
        marcadorElement.zIndex = novoZIndex;
    },

    // Função para auto-incrementar o input de lote se for numérico
    autoIncrementarInputLote: function(numeroLoteAtual) {
        // Verifica se o número do lote é numérico
        const numero = parseInt(numeroLoteAtual);
        if (!isNaN(numero)) {
            // Se for numérico, incrementa 1
            const proximoNumero = numero + 1;
            $('#inputLoteAtual').val(proximoNumero);
        }
        // Se não for numérico, não faz nada (mantém o texto como está)
    },

    // Função para determinar z-index baseado na cor do marcador
    obterZIndexMarcador: function(cor) {
        // Marcadores amarelos ficam por cima de todos os outros
        if (cor && cor.toLowerCase() == '#cc44ff') {
            return 100; // Z-index alto para amarelos
        }
        return 50; // Z-index padrão para outros marcadores
    },

    // Função para recriar marcador editado
    recriarMarcadorEditado: function(dadosMarcador) {
        // Cria HTML do marcador (igual ao marcador normal)
        let el = document.createElement('div');
        el.style.height = '32px';
        el.style.padding = '0 10px';
        el.style.background = dadosMarcador.cor;
        el.style.borderRadius = '10px';
        el.style.display = 'flex';
        el.style.alignItems = 'center';
        el.style.justifyContent = 'center';
        ...
        el.style.cursor = 'pointer';
        el.className = 'marcador-personalizado';
        el.textContent = dadosMarcador.lote;
        
        // Cria marcador avançado
        let marker = new google.maps.marker.AdvancedMarkerElement({
            position: { lat: dadosMarcador.lat, lng: dadosMarcador.lng },
            content: el,
            gmpClickable: true,
            zIndex: MapFramework.obterZIndexMarcador(dadosMarcador.cor)
        });

        // Define propriedades do marcador
        marker.identificadorBanco = dadosMarcador.id;
        marker.numeroMarcador = dadosMarcador.lote;
        marker.quarteirao = dadosMarcador.quarteirao;
        marker.quadra = dadosMarcador.quadra;
        marker.lote = dadosMarcador.lote;
        marker.cor = dadosMarcador.cor;
        ...
        // Adiciona evento de clique (igual ao marcador normal)
        el.addEventListener('click', function(event) {
            event.stopPropagation();
            mostrarTooltipMarcador(marker, event);
        });
    ...
    iniciarDesenhoMarcador: function () {
        ...
        $('#inputQuadraAtual').addClass('modo-insercao');
    ...
    sairModoMarcador: function() {
        ...
        $('#inputLoteAtual').removeClass('modo-insercao');
        $('#inputQuadraAtual').removeClass('modo-insercao');
        this.modoInsercaoMarcador = false;
                ...
                if (response.status === 'sucesso') {
                    response.dados.forEach(desenho => {
                        ...
                        let marker = new google.maps.marker.AdvancedMarkerElement({
                            ...
                            content: el,
                            gmpClickable: true,
                            zIndex: MapFramework.obterZIndexMarcador(desenho.cor)
                        });

                        // Não mostra no mapa por padrão - só adiciona quando quarteirão for selecionado
                        // marker.setMap(MapFramework.map);
    ...
    */
    ...
    marcarLoteComoInserido: function(numeroLote) {
        ...
        // Força uma atualização completa da lista de lotes
        // Isso vai re-executar verificarLoteJaInserido para todos os lotes
        if (typeof popularLotesQuarteirao === 'function') {
            // Recarrega toda a lista para aplicar as verificações
            const quarteiraoData = window.dadosQuarteiraoAtual;
            if (quarteiraoData && quarteiraoData.lotes) {
                popularLotesQuarteirao(quarteiraoData.lotes);
            }
        }
        
        // Também chama a função de atualização se existir
    ...
    passarParaProximoLote: function() {
        ...
        // Procura o próximo lote na ordem visual da lista
        $('.opcao-lote').each(function() {
            if (encontrouAtual && !$(this).hasClass('lote-inserido')) {
                // Este é o próximo lote disponível na ordem visual
                proximoLoteElement = $(this);
        ...
        if (proximoLoteElement) {
            // Extrai o identificador do lote do texto (números e letras)
            const loteTexto = proximoLoteElement.find('.lote-texto').text();
            const match = loteTexto.match(/Lote: ([^|]+)/);
            
            if (match) {
                const numeroLote = match[1].trim();
            ...
            // Se não há mais lotes, pode mostrar uma mensagem ou fazer alguma ação
            console.log('Todos os lotes foram processados ou não há próximo lote disponível.');
        }
    },
