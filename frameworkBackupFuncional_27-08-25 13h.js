...
const MapFramework = {
    ...
    inserirMarcadorPersonalizado: function (latLng) {
        ...
        // Salva no banco e guarda o ID retornado
        this.salvarMarcadorNoBanco(latLng, idQuadra, numeroLote, marker);
        
        // Marca o lote atual como inserido com sucesso (verde)
        this.marcarLoteComoInserido(numeroLote);
        
        // Passa para o próximo lote automaticamente
        this.passarParaProximoLote();
    },

    salvarMarcadorNoBanco: function(latLng, idQuadra, numeroMarcador, marcadorElement) {
        $.ajax({
            ...
            success: (response) => {
                ...
                if (response.status === 'sucesso') {
                    response.dados.forEach(desenho => {
                        ...
                        const coords = JSON.parse(desenho.coordenadas);
                        const numeroMarcador = parseInt(desenho.tipo);
                        if (!coords[0]) return;
                        ...
                        arrayCamadas['marcador_quadra'].push(marker);
                        // Atualiza o maior número já usado para a quadra
    ...
    marcarLoteComoInserido: function(numeroLote) {
        // Encontra o lote SELECIONADO (com classe 'selected') e marca como verde
        $('.opcao-lote.selected').css({
    ...
    passarParaProximoLote: function() {
        ...
        if (proximoLoteElement) {
            // Extrai o número do lote do texto
            const loteTexto = proximoLoteElement.find('.lote-texto').text();
            const match = loteTexto.match(/Lote: (\d+)/);
            
            if (match) {
                const numeroLote = match[1];
