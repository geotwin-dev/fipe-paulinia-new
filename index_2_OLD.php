...
<body>
    ...
    <script>
        ...
        function popularLotesQuarteirao(dadosLotes) {
            ...
            function verificarLoteJaInserido(quadra, numeroLote) {
                ...
                return arrayCamadas.marcador_quadra.some(marker => {
                    return String(marker.quadra) === String(quadra) && 
                    String(marker.numeroMarcador) === String(numeroLote) && 
        ...
        function atualizarListaLotes() {
            console.log('=== INICIANDO atualizarListaLotes ===');
            
            // Verifica se a divCadastro3 está visível
            if (!$('#divCadastro3').is(':visible')) {
                console.log('divCadastro3 não está visível, saindo...');
                return; // Não faz nada se a div não estiver aberta
            }
            
            console.log('Total de marcadores no array:', arrayCamadas.marcador_quadra.length);
            ...
            $('.opcao-lote').each(function() {
                ...
                console.log(`Verificando lote: Quadra=${quadra}, Lote=${lote}`);
                
                // Verifica se este lote ainda está no mapa
                // Converte ambos para string para garantir comparação correta
                const jaInserido = arrayCamadas.marcador_quadra.some(marker => {
                    const match = String(marker.quadra) === String(quadra) && 
                    String(marker.numeroMarcador) === String(lote) &&
                    marker.quarteirao == quarteiraoAtualSelecionado;
                    
                    if (match) {
                        console.log(`MATCH ENCONTRADO: marker.quadra=${marker.quadra}, marker.numeroMarcador=${marker.numeroMarcador}, marker.quarteirao=${marker.quarteirao}`);
                    }
                    
                    return match;
                });
                
                console.log(`Lote ${lote} da quadra ${quadra} - jaInserido:`, jaInserido);
                ...
                if (jaInserido) {
                    ...
                    console.log(`Lote ${lote} marcado como inserido (verde)`);
                } else {
                    ...
                    console.log(`Lote ${lote} voltou ao normal`);
                }
            });
            
            console.log('=== FIM atualizarListaLotes ===');
        }
