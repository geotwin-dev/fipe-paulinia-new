...
try {
    $sql = "SELECT * FROM helpdesk WHERE id = :id AND usuario_email = :usuario_email";
    ...
    // Buscar respostas do admin (se a tabela existir)
    $respostas = [];
    try {
        ...
        // Tabela admin_respostas não existe ainda, continuar sem mostrar respostas
        $respostas = [];
    }
    ...
    if ($respostas) {
        $html .= '
                    <div class="mb-4">
                        <strong class="text-white">Respostas do Suporte:</strong>
                        <div class="mt-2">';
