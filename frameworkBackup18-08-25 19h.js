...
const MapFramework = {
    ...
    inserirMarcadorPersonalizado: function (latLng) {
        ...
        let proximoNumero = (this.marcadoresPorQuadra[idQuadra] || 0) + 1;
        ...
        el.textContent = proximoNumero.toString().padStart(2, '0');
        ...
        marker.numeroMarcador = proximoNumero;
        if (!arrayCamadas["marcador"]) arrayCamadas["marcador"] = [];
        arrayCamadas["marcador"].push(marker);
        adicionarObjetoNaCamada("marcador", marker);
        // Salva no banco
        this.salvarMarcadorNoBanco(latLng, idQuadra, proximoNumero);
        // Atualiza o maior número já usado para a quadra
        this.marcadoresPorQuadra[idQuadra] = proximoNumero;
    },

    salvarMarcadorNoBanco: function(latLng, idQuadra, numeroMarcador) {
        $.ajax({
            ...
            data: {
                ...
                tipo: numeroMarcador // <- aqui vai o número!
            },
            success: function(response) {
                // Pode tratar resposta se quiser
                // Ex: mostrar mensagem de sucesso ou atualizar UI
            },
            error: function() {
        ...
        Object.keys(arrayCamadas).forEach(nomeCamada => {
            arrayCamadas[nomeCamada].forEach(obj => {
                if (obj instanceof google.maps.Polygon || obj instanceof google.maps.Polyline) {
                    ...
                    if(arrayCamadas[nomeCamada] != "quadriculas"){
    ...
    excluirDesenhoSelecionado2: function (cliente, ortofoto) {
        ...
        if (tipo === 'polilinha') {
            ...
            // 2b. Apaga todos os marcadores que pertencem a esta quadra
            arrayCamadas['marcador']
                .filter(marker => parseInt(marker.idQuadra) === identificador)
                .forEach(marker => {
                    marker.setMap(null);
                    this.removerObjetoDasCamadas(marker);
                });
    ...
    carregarMarcadoresSalvos: function (ortofoto) {
        ...
        $.ajax({
            ...
            success: (response) => {
                ...
                if (response.status === 'sucesso') {
                    response.dados.forEach(desenho => {
                        if ((desenho.camada || '').toLowerCase() !== 'marcador') return;
                        const coords = JSON.parse(desenho.coordenadas);
                        ...
                        el.style.fontSize = '16px';
                        el.style.border = '2px solid #fff';
                        el.textContent = numeroMarcador.toString().padStart(2, '0');
                        // Cria marcador avançado
                        let marker = new google.maps.marker.AdvancedMarkerElement({
