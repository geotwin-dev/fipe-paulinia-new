...
<body>
    ...
    <script>
        ...
        // Configurações de tipos de campo por coluna
        const tiposCampos = {
            'data': ['inclusao', 'dt_apro_proj_1', 'dt_habitese_1', 'dt_apro_proj_2', 'dt_habitese_2', 'dt_apro_proj_3', 'dt_habitese_3', 'dt_apro_proj_4', 'dt_habitese_4'],
            'intervalo': ['numero', 'total_construido', 'area_terreno', 'area_excedente', 'area_apro_proj_1', 'area_habitese_1', 'area_apro_proj_2', 'area_habitese_2', 'area_apro_proj_3', 'area_habitese_3', 'area_apro_proj_4', 'area_habitese_4', 'area_construida_a', 'area_construida_b', 'area_construida_c']
        };
        ...
        function criarCamposFiltro(filtroId, coluna) {
            ...
            let tipoCampo = 'texto';
            if (tiposCampos.data.includes(coluna)) {
                tipoCampo = 'data';
            } else if (tiposCampos.intervalo.includes(coluna)) {
                tipoCampo = 'intervalo';
            }
                ...
                case 'intervalo':
                    camposHTML = `
                        <input type="number" class="form-control" placeholder="De" style="width: 100px;"
                               oninput="aplicarFiltro('${filtroId}', '${coluna}', 'intervalo')"
                               id="${filtroId}_de">
                        <span>até</span>
        ...
        function aplicarFiltro(filtroId, coluna, tipo) {
            ...
            switch (tipo) {
                ...
                case 'data':
                    ...
                    // Para datas, usar regex para filtrar intervalo
                    if (dataInicio && dataFim) {
                        valorFiltro = `^(${dataInicio}|${dataFim}|[${dataInicio}-${dataFim}])`;
                    } else if (dataInicio) {
                        valorFiltro = dataInicio;
                    } else if (dataFim) {
                        valorFiltro = dataFim;
                    }
                    break;
                case 'intervalo':
                    ...
                    // Para números, criar regex que funcione com intervalos
                    if (valorInicio && valorFim) {
                        valorFiltro = `^([${valorInicio}-${valorFim}]|${valorInicio}|${valorFim})`;
                    } else if (valorInicio) {
                        valorFiltro = valorInicio;
                    } else if (valorFim) {
                        valorFiltro = valorFim;
                    }
                    break;
            }

            // Aplicar filtro na coluna específica do DataTables
            if (table && table.column(colunaIndex).search() !== valorFiltro) {
