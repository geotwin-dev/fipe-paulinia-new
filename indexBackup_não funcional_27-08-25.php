...
<body>
        ...
        <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
            <div class="container-fluid">
                ...
                <div class="d-flex align-items-center flex-grow-1 gap-2">
                    ...
                    <button id="btnMarcador" class="btn btn-danger d-none">Marcador</button>
    ...
    <script>
        ...
        // Botão Marcador
        $('#btnMarcador').on('click', function() {
            const loteAtual = $('#inputLoteAtual').val();
            const loteSelecionado = $('.opcao-lote.selected').data('lote');
            
            if (!loteAtual || loteAtual.trim() === '') {
                alert('Por favor, selecione um lote primeiro.');
                return;
            }
            
            // Verifica se o texto do input é igual ao lote selecionado
            if (loteAtual.trim() === loteSelecionado.toString()) {
                // Texto igual - entra no modo de inserção imediatamente
                entrarModoInsercaoMarcador();
            } else {
                // Texto diferente - pergunta se realmente quer adicionar com esse lote
                const confirmacao = confirm(`O texto do input (${loteAtual}) é diferente do lote selecionado (${loteSelecionado}).\n\nDeseja realmente adicionar o marcador com o texto "${loteAtual}"?`);
        ...
        // Função para entrar no modo de inserção de marcador
        function entrarModoInsercaoMarcador() {
            // Ativa o modo de inserção
            MapFramework.modoInsercaoMarcador = true;
            
            // Muda o botão para "Sair do modo marcador"
            $('#btnMarcador').removeClass('btn-danger').addClass('btn-warning');
            $('#btnMarcador').text('Sair do modo marcador');
            
            // Adiciona indicador visual no input
            $('#inputLoteAtual').addClass('modo-insercao');
            
            // DESABILITA COMPLETAMENTE o listener global de clique do framework
            if (MapFramework.listenerGlobalClick) {
                MapFramework.listenerGlobalClick.remove();
                MapFramework.listenerGlobalClick = null;
            }
            
            // Desabilita cliques em TODOS os polígonos e linhas durante o modo de inserção
            ...
            // Adiciona listener de clique no mapa para inserir marcadores
            // Usa um nome específico para evitar conflitos com o framework
            MapFramework.listenerMarcador = MapFramework.map.addListener('click', inserirMarcadorNoMapa);
            
            // Muda o cursor do mapa para indicar modo de inserção
            MapFramework.map.setOptions({
                draggableCursor: 'crosshair'
            });
            
            // Adiciona listener para mudanças no input (permitir edição durante o modo)
            $('#inputLoteAtual').off('input.modoMarcador').on('input.modoMarcador', function() {
                // Permite edição do texto durante o modo de inserção
                // O usuário pode alterar o texto e clicar no mapa para inserir com o novo texto
            });
            
            // Mostra mensagem de instrução
            alert('Modo de inserção de marcador ativado!\n\nClique em qualquer lugar dentro do quarteirão selecionado para adicionar um marcador.\n\nVocê pode editar o texto do input antes de clicar no mapa.\n\nPara sair do modo, clique em "Sair do modo marcador".');
        }

        // Função para inserir marcador no mapa
        function inserirMarcadorNoMapa(event) {
            // Mostra apenas o clique
            console.log('CLIQUE NO MAPA:', event.latLng);
            
            // Verificação de segurança
            if (!MapFramework.modoInsercaoMarcador || !MapFramework.listenerMarcador) {
                return;
            }
            
            // Previne que o evento se propague para outros listeners
            event.stop();
            
            const loteAtual = $('#inputLoteAtual').val();
            const quarteiraoSelecionado = $('.opcao-quarteirao.selected input[name="quarteirao"]').val();
            
            if (!loteAtual || !quarteiraoSelecionado) {
                alert('Erro: Lote ou quarteirão não selecionado.');
                return;
            }
            
            // Verifica se o clique foi dentro do quarteirão selecionado
            const quarteirao = MapFramework.obterQuarteiraoPorId(quarteiraoSelecionado);
            
            if (!quarteirao || !quarteirao.polygon) {
                alert('Erro: Quarteirão não encontrado.');
                return;
            }
            
            // Verifica se a biblioteca TURF está disponível
            if (typeof turf === 'undefined') {
                alert('Erro: Biblioteca TURF não está disponível. Recarregue a página.');
                return;
            }
            
            // Verifica se o ponto clicado está dentro do quarteirão usando TURF
            const pontoClicado = event.latLng;
            
            // Converte o quarteirão para GeoJSON para usar com TURF
            const path = quarteirao.polygon.getPath();
            const coordenadasQuarteirao = [];
            
            for (let i = 0; i < path.getLength(); i++) {
                const latLng = path.getAt(i);
                coordenadasQuarteirao.push([latLng.lng(), latLng.lat()]);
            }
            
            // Fecha o polígono (primeira coordenada = última coordenada)
            if (coordenadasQuarteirao.length > 0) {
                coordenadasQuarteirao.push(coordenadasQuarteirao[0]);
            }
            
            // Cria o polígono GeoJSON para usar com TURF
            const quarteiraoGeoJSON = turf.polygon([coordenadasQuarteirao]);
            const pontoGeoJSON = turf.point([pontoClicado.lng(), pontoClicado.lat()]);
            
            const pontoDentro = turf.booleanPointInPolygon(pontoGeoJSON, quarteiraoGeoJSON);
            
            if (!pontoDentro) {
                alert('Erro: Clique deve ser dentro do quarteirão selecionado!');
                return;
            }
            
            // Cria o marcador avançado personalizado (bolinha vermelha com texto)
            ...
            contentDiv.style.cssText = `
                ...
                user-select: none;
            `;
            contentDiv.textContent = loteAtual;
            
            // Cria o marcador com o conteúdo
            const marcador = new google.maps.marker.AdvancedMarkerElement({
                position: pontoClicado,
                content: contentDiv,
                title: `Lote: ${loteAtual}`,
                gmpClickable: false,
                zIndex: 1000
            });
            
            // Adiciona o marcador à camada de marcadores
            adicionarObjetoNaCamada('marcador', marcador);
            
            // Verifica se a camada de marcadores está visível
            const chkMarcadores = document.getElementById('chkMarcadores');
            if (chkMarcadores && !chkMarcadores.checked) {
                chkMarcadores.checked = true;
            }
            
            // Marca a linha do lote como verde (concluída)
            const loteSelecionado = $('.opcao-lote.selected');
            loteSelecionado.css('background-color', '#d4edda');
            loteSelecionado.css('border-color', '#c3e6cb');
            loteSelecionado.css('color', '#155724');
            
            // Avança automaticamente para o próximo lote
            avancarParaProximoLote();
            
            // Mostra mensagem de sucesso
            alert(`Marcador adicionado com sucesso para o lote ${loteAtual}!\n\nAvançando automaticamente para o próximo lote...`);
        }

        // Função para avançar automaticamente para o próximo lote
        function avancarParaProximoLote() {
            const loteAtual = $('.opcao-lote.selected');
            const proximoLote = loteAtual.next('.opcao-lote');
            
            if (proximoLote.length > 0) {
                // Seleciona o próximo lote
                proximoLote.trigger('click');
            } else {
                // Se não há próximo lote, volta para o primeiro
                const primeiroLote = $('.opcoesLotes .opcao-lote').first();
                if (primeiroLote.length > 0) {
                    primeiroLote.trigger('click');
                }
            }
            
            // Limpa a seleção visual do lote anterior (remove a flecha)
            $('.lote-flecha').html('&nbsp;&nbsp;');
            
            // Adiciona a flecha ao novo lote selecionado
            const novoLoteSelecionado = $('.opcao-lote.selected');
            if (novoLoteSelecionado.length > 0) {
                novoLoteSelecionado.find('.lote-flecha').html('>');
            }
        }

        // Função para sair do modo de inserção de marcador
        function sairModoInsercaoMarcador() {
            // Desativa o modo de inserção
            MapFramework.modoInsercaoMarcador = false;
            
            // Restaura o botão original
            $('#btnMarcador').removeClass('btn-warning').addClass('btn-danger');
            $('#btnMarcador').text('Marcador');
            
            // Remove o listener específico de marcador
            if (MapFramework.listenerMarcador) {
                MapFramework.listenerMarcador.remove();
                MapFramework.listenerMarcador = null;
            }
            
            // RESTAURA o listener global de clique do framework
            if (MapFramework.map) {
                ...
                // Cria um novo listener global
                MapFramework.listenerGlobalClick = MapFramework.map.addListener('click', () => {
                    MapFramework.desselecionarDesenho();
                });
            }
            
            // Restaura o cursor do mapa
            MapFramework.map.setOptions({
                draggableCursor: null
            });
            
            // Remove o listener do input
            $('#inputLoteAtual').off('input.modoMarcador');
            
            // Remove indicador visual do input
            $('#inputLoteAtual').removeClass('modo-insercao');
            
            // REABILITA cliques em TODOS os polígonos e linhas
            Object.keys(arrayCamadas).forEach(camada => {
                if (arrayCamadas[camada] && Array.isArray(arrayCamadas[camada])) {
                    arrayCamadas[camada].forEach(obj => {
                        if (obj instanceof google.maps.Polygon || obj instanceof google.maps.Polyline) {
                            // Restaura a propriedade clickable original
                            obj.setOptions({
                                clickable: true
                            });
                        }
        ...
        // Evento para o botão Marcador quando está no modo de inserção
        $(document).on('click', '#btnMarcador', function() {
            if (MapFramework.modoInsercaoMarcador) {
                // Se está no modo de inserção, sai dele
                sairModoInsercaoMarcador();
            } else {
                // Se não está no modo de inserção, entra nele (lógica já implementada acima)
                const loteAtual = $('#inputLoteAtual').val();
                const loteSelecionado = $('.opcao-lote.selected').data('lote');
        ...
        // Proteção simples: evita que os botões de desenho sejam clicados durante o modo de marcador
        $(document).on('click', '#btnIncluirPoligono, #btnIncluirLinha', function() {
            if (MapFramework.modoInsercaoMarcador) {
                alert('Você está no modo de inserção de marcadores. Saia deste modo primeiro para desenhar quadras ou lotes.');
                return false;
            }
        });

        // Inicializa as variáveis de modo de inserção no framework
        ...
        function destacarQuarteiraoSelecionado(nomeQuarteirao, idQuarteirao) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#btnMarcador').removeClass('d-none');
                    $('#inputLoteAtual').show();
        ...
        function popularLotesQuarteirao(dadosLotes) {
            ...
            Object.keys(lotesPorQuadra).forEach(quadra => {
                const lotes = lotesPorQuadra[quadra];
                
                lotes.forEach((lote, index) => {
                    const opcao = $(`
                        <div class="opcao-lote" data-quadra="${quadra}" data-lote="${lote.lote}">
                            <div class="d-flex align-items-center">
                                <span class="lote-flecha me-2" style="color: #007bff; font-weight: bold;">${index === 0 ? '>' : '&nbsp;&nbsp;'}</span>
            ...
            if (container.find('.opcao-lote').length > 0) {
                const primeiroLote = container.find('.opcao-lote').first();
                primeiroLote.trigger('click');
            }
        }
