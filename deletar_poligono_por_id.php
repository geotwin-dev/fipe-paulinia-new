...
if (!isset($data['id'])) {
    $response['message'] = 'ID do polígono não fornecido.';
...
try {
    ...
    // Ocultar polígono usando ID
    $sql = "UPDATE poligonos_pdf SET visibilidade = 0 WHERE id = :id";
    error_log('🔍 SQL: ' . $sql);
    ...
    if ($rowCount > 0) {
        ...
        $response['message'] = 'Polígono ocultado com sucesso.';
        error_log('✅ Polígono ocultado com sucesso, ID: ' . $id);
    } else {
        $response['message'] = 'Nenhum polígono encontrado com o ID fornecido.';
        error_log('⚠️ Nenhum polígono encontrado com ID: ' . $id);
    }
